...
cget() { curl -sf "http://$consul_addr/v1/kv/service/vault/$1?raw"; }
...
if [ ! $(cget root-token) ]; then
  ...
  cat /tmp/vault.init | grep '^Unseal' | awk '{print $4}' | for key in $(cat -); do
    curl -fX PUT $consul_addr/v1/kv/service/vault/unseal-key-$COUNTER -d $key
    COUNTER=$((COUNTER + 1))
  done
  ...
  curl -fX PUT $consul_addr/v1/kv/service/nodejs/show_vault -d "true"
  curl -fX PUT $consul_addr/v1/kv/service/nodejs/vault_files -d "aws.html,generic.html"
else
...
vault write mysql/config/connection \
    connection_url="root:rootpw@tcp(mysqldb:3306)/"
